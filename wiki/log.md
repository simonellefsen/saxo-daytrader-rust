...
- `AppState` retains persisted reads, tab gating, data freshness, regime collection, scheduling, report generation, manager gates, queues, and Saxo behavior.
...
- Provider requests, report scheduling, manager gates, queues, prechecks, and Saxo behavior are unchanged.
...
- **The first pass missed `src/ui.rs`; corrected the same day.** Its eight panel decoders read a staged manager or diary sub-document and swallow the decode error with `.ok()`, so a null blanks Cash Deployment or Instrument Quarantine with *no warning at all* — quieter than the boundaries fixed first, which at least logged a degradation. Every field on those panel payloads is defaulted or `Option`, so the whole panel rests on defaults and one null takes all of it. Their tests go through the real panel functions, not the decoder, so reverting a call site fails them.
...
## [2026-08-31] risk | Live currency concentration measured and surfaced

- Closed U10's last engineering slice. `PositionExposure::currency_exposure_json` buckets invested value by currency; the Cash Deployment panel renders share, position count, totals, and unvalued/unmapped caveats beside the drawdown guardrail.
- Shares are of *invested value*, not of the book: the exposure map holds positions plus active BUY reservations and no cash. The scope string says so at both emission sites.
- The panel states the distinction the policy decision turns on: `strategy.concentration.max_assets_per_currency` counts **assets, not value**, so a cap of 5 would have permitted exactly the 63%-USD book that exists. It measures and gates nothing.
- The row is unconditional, unlike the guardrail rows beside it. Those appear when they act; this one exists because currency was the exposure nothing showed at all, and a threshold-triggered row would need a threshold nobody has decided.
- **Found while wiring it: `xome` had no currency.** U11 moved 20 symbols -- the whole Swedish leg, a tenth of the universe -- from `:xsto` to `:xome` and updated the Markov exchange-id map but not `currency_for_exchange`. It survived a month because neither failure is loud: `order_currency` falls back to DKK for an unmapped exchange, and the currency cap refuses to evaluate rather than erroring. Enabling that cap while any Swedish position was held would have blocked every BUY.
- Both spellings now map to SEK, pinned by a test that reads the analysis universe out of both shipped configs and asserts every suffix it references has a currency -- so the next exchange added to the universe fails in CI rather than in FX math.
- **Traced the blast radius rather than assuming it: nothing was mis-valued and no backfill is needed.** Held-position valuation reads Saxo's own currency from `broker_position_snapshots`; the trade ledger prefers the order and broker payload currencies ahead of this map; and the counterfactual and missed-trade shadow ledgers compute in local currency, so FX never enters them. What was genuinely exposed was latent, not active: the currency cap refuses to evaluate a symbol it cannot classify, so switching it on while holding a Swedish name would have blocked every BUY. Recorded as a table in [urgent-todo](urgent-todo.md)'s Currency Exposure section.
- The lesson is the shape rather than the size. The gap survived a month because both of its failure modes are quiet: one falls back to a plausible default, the other declines to act.
- Remaining on U10 is the policy choice alone, and it is the operator's.

## [2026-08-31] reliability | Reviewed the branch and fixed three defects

- Reviewed the whole branch before adding more, rather than after. Three real defects, all in code written the same day.
- **`decode_each` warned once per row.** The tolerant path fires on a defaulted non-Option field carrying a null, and a field that is null in one row is usually null in most of them -- a 20-row Decision Report list with a null `model` would have written 20 identical warnings per API call, burying the boundary name the warning exists to report. A batch now warns once and reports how many rows needed it.
