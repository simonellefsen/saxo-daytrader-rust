...
- The dashboard derives bounded `Learning Memory` from safe reflection actions. A lesson repeated across two distinct reflections is `stable`; one recent observation is `emerging`; emerging lessons expire after 7 days and stable lessons after 21 days. `stale` lessons remain operator-visible but are excluded from `/api/hermes/context`, preventing outdated conditions from influencing future advice. The projection is read-only and contains no raw reflection/provider/broker payloads or mutable strategy state.
...
- The dashboard's read-only `Baseline Promotion Evidence` pack joins the promoted baseline's source experiment to only those locally recorded Trading Manager runs that used its exact overlay, their linked execution outcome counts, and bounded portfolio observations. Portfolio return, drawdown, cash utilization, and sufficient-sample zero-risk-free Sharpe are observational and not causal claims. The pack cannot change a baseline, experiment, configuration, or broker behavior and excludes raw proposal/provider/broker content.
- The dashboard's read-only `Gate Replay` panel and `/api/decision/gate-replay` re-evaluate only an isolated historical target gate from sanitized `trading_manager_runs` snapshots. The initial scenarios compare a fresh-long Markov starter threshold of `0.25` and a BUY technical-confluence threshold of `4`. A replayed target-gate clear is evidence only, never a full approval or execution prediction; all other gates, broker state, capital, and market conditions remain outside the comparison. It cannot call a provider or Saxo, create orders, or mutate configuration.
