...
- 2026-07-13: Continued the P0 dashboard read-model split. Tab-exclusive collections for Hermes, Execution detail, Markov, Quiver, Watchlists, End-of-Day, and decision-pulse status now load only when their tab is selected; shared overview/operations health inputs remain available across views.
...
| P0 | UI performance | Continue splitting dashboard reads per tab. | The first six slices landed 2026-07-13: Decision Report lists exclude prompt/request/response/report JSON except for active detail; portfolio history loads only on Performance; tab-exclusive collections load only on their own tabs; Execution orders, Markov signals, and Quiver signals are server-paginated with bounded shared views. Remaining work: paginate remaining long shared tables and add detail endpoints. |
