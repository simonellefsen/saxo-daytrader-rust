...
| P1 | Markov coverage | Continue fixing the assets that fail instrument resolution on daily Markov runs: negative caching and share-class compact symbol variants landed 2026-07-09; remaining work is correcting genuinely wrong exchange mappings and non-class ticker aliases where possible. | 19% of the universe was missing regime signal coverage. Weekly negative caching stops repeated dead-end lookups, and share-class variants should recover many `*-A`/`*-B` Nordic and Berkshire-style rows without weakening exchange checks. |
