...
| P1 | Testing | Continue expanding workflow tests. Decision action, scheduled-report freshness, Hermes conservative self-check, execution-queue admission, and database-backed Trading Manager queue persistence guards have landed; next slice is a database-backed execution-order claim race fixture with no broker HTTP. | Protects the most critical cross-module workflows. |
