...
| P1 | Signal coverage | Widen the daily-indicator universe beyond current holdings: nightly runs cover exactly 20 assets (the 13 broker holdings plus a few legacy symbols), so watchlist candidates like DSV:xcse have no daily technicals and every candidate BUY gets Hermes stand_down with "daily technical data is missing, 0/3 confluences" (reports 170 and 174). Include the active watchlist in the nightly run or compute indicators on demand for candidate symbols. | With cash at 10.4k and BUY candidates structurally unable to pass the confluence gate, the system cannot rotate into new names — it can only add to existing holdings or sit still while the book decays. |
