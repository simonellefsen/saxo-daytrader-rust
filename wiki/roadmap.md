...
| P1 | Risk guardrail | Extend the monthly-loss circuit breaker (landed 2026-07-08) with an operator acknowledgment path for resuming BUYs before month end. Slack alerting for activation/deactivation landed 2026-07-09. | The breaker currently informs through manager runs, the prompt, and Slack; the remaining risk is lack of an explicit operator path for exceptional resumption. |
| P1 | Accounting integrity | Expand the new overview `integrity` checks with broker exposure aggregate reconciliation and operator acknowledgment for high-severity violations. UI surfacing and Slack routing landed 2026-07-09. | The May position_lots corruption (3.2M DKK cost basis on 31 shares) sat unnoticed for a month; invariants with alerts catch the next one in hours. |
| P1 | Markov coverage | Continue fixing the 38 assets that fail instrument resolution on every daily Markov run (AKRBP:xosl, ALFA:xsto, ALK-B:xcse, ABB:xsto, ...): negative caching with weekly retry landed 2026-07-09; remaining work is correcting Saxo symbology mappings for Nordic/EU exchange suffixes where possible. | 19% of the universe — concentrated in exactly the Nordic/EU names the EU-open pulse trades — has no regime signal at all. Weekly negative caching stops repeated dead-end lookups while preserving a slow retry path. |
| P1 | Instrument quarantine | Expand the derived quarantine with an operator acknowledge/override path. Slack alerting for active derived quarantines landed 2026-07-09; consider persisting signatures if the derived read model becomes too expensive or needs manual notes. | Known-broken instruments waste daily order capacity, pollute execution stats, and re-teach the model nothing; the manual do-not-trade list idea needs an automatic feeder. |
...
| Idea | Shape | Measurement |
| --- | --- | --- |
| Unstick the experiment review queue | Four one-variable proposals have sat in `pending_review` since as early as 2026-06-16 (Markov age gate, min_confluences 3→4, and two near-duplicate cash-buffer raises) while reflections run nightly. Add pending-review aging alerts through the Slack path, a weekly review digest, auto-expiry with a "stale, superseded by market conditions" status after N days, and near-duplicate merging before insert. | Median time from proposal to decision; zero proposals older than 14 days; the self-improvement loop actually completes its cycle instead of accumulating unreviewed hypotheses. |
