...
| P0 | Decision prompt integrity | Landed 2026-07-16/17: `watchlists_payload` entries older than `position_decision_stale_after_days` (7d) keep their symbol as a universe member (the ~170-symbol May sentiment archive IS the analysis universe — Markov and daily indicators build their asset lists from this payload) but are stripped to a bare `{symbol, quote_status: stale_history/stale_quote_dropped}` row: months-old prices, sentiment, and "Existing portfolio holding" rationales no longer reach prompts as current data. Follow-ups: verify the next decision prompt has no phantom-holding sentiment, and give the universe an explicit configured source instead of resting on fossil sentiment rows. | 5 of the week's 11 Trading Manager skips were phantom NNIT/ORSTED SELLs (reports 166/168/170/172/174) — the broker-authoritative guard correctly blocked them, but the model kept burning suggestion slots on positions that did not exist because the prompt told it they did. |
...
| P2 | Risk governance | Landed 2026-07-21: documented and tiered the monthly-loss policy. A soft floor at `-25,000 DKK` reduces the cycle-wide BUY budget to 50%; the deliberate hard halt remains at `-50,000 DKK`. The soft floor must be negative and less severe than the hard floor or it is disabled. Follow-up: observe the soft tier in live manager runs before changing either threshold or multiplier. | Replaces a binary response with a visible, deterministic reduction while preserving the hard capital stop and unrestricted defensive SELLs. |
...
| Post-trade attribution | Execution order details now attach each order to its report, Trading Manager decision, Hermes advice, and report-time technical/Markov/capital snapshots. Remaining work: attach realised P/L and holding-period outcomes to that immutable decision context. | Detect which pulses and filters actually add value. |
...
- Landed 2026-07-21: Execution attribution links order -> report -> Trading Manager -> Hermes -> report-time Markov/technical/capital context. It prefers stored manager snapshots and labels current latest-signal reads as a fallback for historical records; remaining work is realised-P/L outcome attribution.
