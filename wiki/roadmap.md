...
| Unknown-state order timeout handling | Phase 1 landed 2026-07-21: an order-placement timeout or `TradeNotCompleted` now becomes `broker_state_unknown`, retains the successful precheck plus `x-request-id`/`ExternalReference`, blocks automatic retry, preserves SELL reservations, emits a dedicated audit event, and appears as an integrity/UI warning. Follow-up: add a broker-authored reconciliation lookup or ENS activity replay before an operator can clear/requeue the hold. | Eliminates the double-order failure mode now; later reconciliation makes the outcome provable without manual broker inspection. |
