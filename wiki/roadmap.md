...
| P0 | UI performance | Continue splitting dashboard reads per tab. | The first two slices landed 2026-07-13: Decision Report lists exclude prompt/request/response/report JSON except for active detail, and portfolio history now loads only on the Performance tab at the selected range. Remaining work: stop loading unrelated tab datasets, then add server-side pagination/detail endpoints. |
