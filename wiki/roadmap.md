...
| P0 | UI performance | Continue splitting dashboard reads per tab. | The first three slices landed 2026-07-13: Decision Report lists exclude prompt/request/response/report JSON except for active detail; portfolio history loads only on Performance; and tab-exclusive Hermes, Execution detail, Markov, Quiver, Watchlist, EOD, and pulse collections load only on their own tabs. Remaining work: paginate long shared tables and add detail endpoints. |
