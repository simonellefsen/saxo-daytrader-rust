...
| P1 | Accounting integrity | Reconcile local lot accounting with the broker book: `position_lots`/`position_snapshots` still hold the full 2026-05-18 import (18 symbols incl. TSLA×163, NOVOb×235, MSTR×48, ~740k DKK basis) while the broker-authoritative snapshot shows a mostly different 13-symbol portfolio (AAKI, ADS, AMAT, AMGN, ARM, ASML, BAC, CSCO…; AMD 7 vs local 5). Bootstrap fresh lots from broker open positions (or a new Saxo positions export), mark superseded May-18 rows excluded, and add an invariant alert when local open quantity diverges from broker quantity per symbol. | Any SELL of a position bought outside the ledger will look up cost basis against stale or missing lots, silently corrupting realised P/L again — the exact failure class repaired on 2026-07-08. The uncommitted SELL final-fill fixture reads "latest position_snapshots cost basis", which today means May 18. |
