...
| --- | --- | --- | --- |
| P0 | Security | Redact the database connection string in the dashboard Runtime panel: `DashboardView.db_label` is set from the raw `db_url`, so the full `postgresql://daytrader:PASSWORD@...` including the password renders on the Overview page (screenshot 2026-07-11) even though `redacted_database_url` exists in `state.rs`. Audit other surfaces for the same leak. | The dashboard is reachable through the public ngrok route behind OAuth; credentials must never render in HTML regardless. |
| P0 | UI performance | Stop loading everything for every view: `load_dashboard` unconditionally fetches `decision_report_items(20)` — whose SELECT includes prompt_text/request_json/response_json/report_json at ~1 MB per row (the 18-row table is 19 MB) — plus 5,000 rows of `portfolio_value_history` on every page render of every tab. Measured 2026-07-11: each view takes 0.9-2.1 s server-side and `?view=prompts` ships 1.0 MB of HTML. Split into per-tab read models that fetch only what the tab renders, load large JSON/prompt payloads on demand behind detail endpoints, and paginate long tables. | This is the direct cause of the sluggish UI; the fix also cuts database load and makes the 10-minute scheduler pods cheaper neighbors. |
| P0 | Deploy provenance | Bake the git commit SHA into the binary at build time, expose it via `/api/health`, and make `make post-deploy-smoke` fail when the running SHA is not an ancestor-inclusive match for the local HEAD being deployed. Evidence: on 2026-07-09 the cluster ran two Trading Manager cycles (08:20, 14:51) from an image built without the previous day's monthly-loss breaker and commission floor — four BUYs executed that both safeguards would have blocked (small clips of 1.3k-3.7k DKK during an active breaker). The existing image-name drift guard cannot catch a stale build of the right name. | Safety features must not silently disappear for a working day because a deploy was cut from a stale tree. |
...
| P1 | Data hygiene | Enforce the configured history caps and reclaim bloat: `scheduler_cycle_history` holds 9,228 rows / 51 MB despite `history_max_rows: 250` (retention is never applied), and `audit_log` is 65 MB with zero live tuples. Add a scheduler-driven prune step honoring `history_max_rows`/`history_retention_days`, plus a periodic VACUUM (or one-off VACUUM FULL during quiet hours) for the two bloated tables. | Cycle JSON at ~5.5 KB/row grows unbounded and slows every scheduler-history query; dead space inflates backups and restore rehearsals. |
...
| P1 | Testing | Continue expanding workflow tests after the first decision-action guard landed 2026-07-10; next slices are scheduled report, Hermes advice, Trading Manager queueing, and execution queue dry-run paths. | Protects the most critical cross-module workflows. |
