...
| P0 | Decision prompt integrity | Stop recycling stale decision-history sentiment into new prompts: watchlist quote entries carry `decision` blobs from old reports, so NNIT/ORSTED still appear with `"rationale": "Existing portfolio holding…"`, `"source": "portfolio_default"` and quotes stamped 2026-06-24 despite the broker no longer holding them. Derive holding-status annotations from broker-authoritative positions at prompt-build time and age-gate or drop sentiment/quote blobs older than N days. | 5 of the week's 11 Trading Manager skips were phantom NNIT/ORSTED SELLs (reports 166/168/170/172/174) — the broker-authoritative guard correctly blocked them, but the model keeps burning suggestion slots on positions that do not exist because the prompt tells it they do. |
| P1 | Accounting integrity | Broker-basis fallback landed 2026-07-16: `latest_position_cost_basis` now falls back to the broker-authoritative open price (including costs, FX-converted) whenever the local snapshot is missing or carries no usable basis, and warns loudly if neither source exists — a SELL of an outside-ledger position can no longer book its full proceeds as realised gain. Remaining work: (a) run the prepared broker-bootstrap SQL (`scripts/broker_bootstrap_20260716.sql`, blocked by session permissions) to give the 13 live positions real snapshot/lot rows and exclude the stale 2026-05-18 import; (b) write snapshot/lot rows at BUY-fill time so future positions never depend on the fallback; (c) add an invariant alert when local open quantity diverges from broker quantity per symbol. | Every position acquired after 2026-05-18 (in-ledger DANSKE/CHEMM/AMGN buys included — BUY fills never wrote snapshots) had NO local basis, so any SELL booked cost basis 0 — the exact failure class repaired on 2026-07-08, waiting to fire on the first flatten exit. |
