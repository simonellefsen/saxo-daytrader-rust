...
| P2 | Risk governance | Document and tier the monthly-loss breaker threshold: commit e80621e raised `monthly_loss_halt_dkk` from −10,000 to −50,000 with no rationale note; month P/L reached −35.1k this week, so the original threshold would have halted BUYs. Add the rationale as a config comment and consider a tiered response (e.g. halve BUY sizing at −25k, hard halt at −50k). | A 5× loosening of the primary capital guardrail should be a deliberate, documented risk decision — and a soft intermediate tier keeps the system trading smaller rather than binary on/off. |
