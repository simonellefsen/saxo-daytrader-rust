...
| P0 | Decision prompt integrity | Landed 2026-07-16: `watchlists_payload` now drops quote- and decision-derived entries older than `position_decision_stale_after_days` (7d) — orphaned `portfolio_price_snapshots` rows (15 rows dating to 2026-05-07, exactly the phantom NNIT/ORSTED/TSLA-era symbols), unbounded-age `latest_symbol_decisions` blobs, and the entire May-5-8 `swing_sentiment_snapshots` residue (1,068 rows) no longer reach prompts or the Markov/indicator universe. Follow-up: verify the next decision prompt contains no phantom-holding sentiment, then consider deleting the orphaned rows outright. | 5 of the week's 11 Trading Manager skips were phantom NNIT/ORSTED SELLs (reports 166/168/170/172/174) — the broker-authoritative guard correctly blocked them, but the model kept burning suggestion slots on positions that did not exist because the prompt told it they did. |
