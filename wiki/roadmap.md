...
| Post-trade attribution | Execution order details now attach each order to its report, Trading Manager decision, Hermes advice, report-time technical/Markov/capital snapshots, and reconciled fill-ledger outcome. SELLs show realised P/L/costs; BUYs show only position-book outcome. Remaining work: 1-day/5-day holding-period outcomes and cross-order position lifecycle attribution. | Detect which pulses and filters actually add value. |
