...
| --- | --- | --- | --- |
| U1 | ~~Finish broker-hosted protective stops (SIM-first)~~ **Landed 2026-09-03** | Downside protection currently depends entirely on the model proposing a SELL at one of two weekday pulses and that SELL clearing the technical gate. Nothing exits between pulses, and nothing exits across a weekend. The 2026-07-25 work landed a read-only coverage audit plus a manually confirmed SIM lifecycle test; the automatic path after a confirmed BUY fill is still unbuilt. `strategy.ladder.submit_stop_loss_after_fill` is still `false` and unread; `stop_loss_atr_multiple` (2.0) was equally dead until slice 1 wired it into the proposed stop level. The roadmap's own live evidence records a −24.4k DKK week in which the defensive exits did not fire. | **Closed 2026-09-03 against measured evidence, after its own text had gone stale.** The row still claimed `protective_stop_prechecks` and `protective_stop_lifecycle_tests` were "both still empty -- the broker path has never executed once" and that `strategy.ladder.submit_stop_loss_after_fill` was "still `false` and unread". Neither holds: those tables carry **223** and **212** rows with the newest from 2026-09-02, the switch is `true` in both shipped configs and the live ConfigMap and is classified *enforced* by the contract audit, and 173 stop orders have been placed (128 cancelled and replaced by the ratchet sweep, 28 filled, 17 working). **All 17 held positions carry a broker-working stop at full quantity**, and **24 of 28 stop fills executed outside any pulse window** -- the "nothing exits between pulses" premise this item was written on. The automatic path after a BUY fill works: of 30 genuinely new positions since the sweep landed, **25 were stopped within 30 minutes** and the typical latency is 21 minutes, two scheduler cycles. **One caveat is recorded rather than resolved:** four positions waited between 3 hours and 4 days for a stop, and all four fall inside 2026-08-13 to 08-17; everything before and after is 17-21 minutes. The cause is unknown and cannot now be recovered, because `scheduler_cycle_history` retains 250 rows reaching back only to 2026-09-01. It is not recurring. Point 7, ENS streaming, was always scoped out of this item as a latency optimisation and already sits on the [roadmap](roadmap.md) as *ENS activities subscription for fills*; its recovery half runs today as the daily `ens_activity_backfill`. |
...
| U3 | Reconcile the Hermes goal contract with enforced reality | `AppState::hermes_goal_contract_value` sent Hermes `max_drawdown: 0.20`, `min_sharpe: 1.0`, `slippage_tolerance: 0.02`, and `max_positions` as *constraints*. Only the monthly-loss DKK floors were enforced in `src/trading_manager.rs`. Portfolio drawdown was computed but consumed only by display and evidence packs, so Hermes reasoned about a risk envelope the runtime did not apply. The return half of this was fixed 2026-07-25 — see [Return Goal](#return-goal). | Landed 2026-07-26. `src/drawdown_guard.rs` makes `max_drawdown` real: a soft band reduces the cycle BUY budget, a hard floor suspends new BUYs, SELLs are never blocked. The contract now reads its limit from `strategy.capital.drawdown_halt_pct`, the same key the gate applies, so the advertised and enforced numbers cannot drift. `strategy.swing.max_holdings` now applies the published `max_positions` constraint to new-symbol BUYs using persisted positions plus same-cycle reservations. Every remaining objective and constraint carries an explicit `enforcement` status (`runtime_enforced` / `evaluation_only` / `structural` / `documentation` / `not_enforced`), and a test fails the build if a field is added without one. `gas_reserve` was deleted as a crypto-template leftover. **Remaining debt, now named rather than implied:** `slippage_tolerance` and `require_backtest_before_activation` are declared `not_enforced`. **Audited 2026-08-31 and one entry corrected.** The slippage detail claimed "No cost model exists, so slippage is never estimated before queueing" — false since the cost guard shipped: `cost_guard_gate` estimates one-way slippage as `strategy.estimated_slippage_bps` (8 bps) of order value, adds the exchange minimum round-trip commission scaled by `strategy.cost_guard_multiple`, and refuses a BUY whose verified reward target cannot clear that floor. Slippage is modelled *and* gating; what remains unenforced is only the contract's own 2% tolerance, which is never compared against anything, so no check would fire if realised slippage approached it. The status stays `not_enforced` and the detail now says which mechanism actually runs, pinned by a test that fails if the text stops naming it. **This is U3's own failure class turned inward** — the record that exists to keep the advertised/enforced boundary honest had itself drifted. Understating enforcement is the milder direction, but it is the same defect. The other six entries were re-checked against the code and hold: `max_holdings` is loaded and gated, `experiment_overlays_allowed` has a test proving a live/LIVE combination is refused, the one-variable rule is an allowlist, and the drawdown guardrail was independently re-verified the same day. `require_backtest_before_activation` remains accurate — no backtest engine exists. |
...
| U9 | ~~Portfolio is 1.4% away from a full BUY halt~~ **Resolved 2026-08-04 by operator decision: halt widened to 25%** | The drawdown guardrail landed 2026-07-26 and stayed in its soft band throughout, tightening steadily: 16.57% (07-29) → 17.27% → 18.25% → 19.04% → 18.999% (07-31) → **19.37% (08-04)** against a 20.00% halt. Peak 297,463 DKK (2026-06-30), current ~240,300. The halt would have fired at 237,970 — under 1% away. The guardrail was working exactly as designed; what was undecided was what should happen when it tripped. See [Drawdown Approach](#drawdown-approach). | **Operator chose to widen `strategy.capital.drawdown_halt_pct` from 0.20 to 0.25** (halt moves 237,970 → 223,097 DKK). The **soft band stays at 0.10** deliberately — it is already active and halving the cycle BUY budget, so caution is retained rather than removed; only the hard stop moved. `DEFAULT_HALT_PCT` in `src/drawdown_guard.rs` stays at 0.20 on purpose: it is the fallback for missing config, and falling to the *stricter* value is the right direction for a risk control. **The change surfaced two real drifts in the Hermes goal contract**, both in `deploy/k8s/base/hermes.yaml`'s `SELF_IMPROVEMENT_GOAL.yaml` — a static file mounted into the Hermes pod at `/opt/daytrader-context`, and therefore a *second copy* of the contract that the U3 "reads the same key so they cannot drift" guarantee does not cover. It still carried `max_drawdown: 0.20` (would have told Hermes a limit the runtime no longer enforces — precisely the U3 failure class) and `gas_reserve: 0.05`, which U3 recorded as deleted but only removed from the Rust side. Both fixed. **Still open, and unchanged by this decision: there is no re-entry rule.** Widening moves the cliff; it does not define how a halted book resumes. That remains a roadmap item and should be settled before the wider floor is ever reached. |
| U10 | The book is 63% unhedged USD and the runtime reports its FX exposure as exactly zero — **realised split landed 2026-08-02, live concentration measured 2026-08-31, policy open** | Live exposures: **USD 87,892 DKK (63%)**, DKK 41,889 (30%), NOK 9,323 (7%). USD/DKK fell from 7.0215 (07-02) to 6.4837 (08-02) — **−7.66% in one month** — against a book whose reporting currency is DKK. `trade_ledger.fx_gain_dkk` is a **hardcoded `0` literal** in the `INSERT` at `src/saxo_order.rs:2506`, and `price_gain_dkk` is bound to `realised_gain_dkk`, so every sale reports 100% price / 0% FX by construction. The real formula exists only in the retired Python (`src/saxo_daytrader_xai/tax_engine.py:273`). Currency is the one exposure nothing gates, nothing measures, and no signal source (Markov, Quiver, indicators, editorial) observes. See [Currency Exposure](#currency-exposure). **Measurement landed 2026-08-02.** `crate::fx::split_realised_gain` decomposes the realised gain exactly: price is `(net_local − cost_local) × sale_rate`, currency is `cost_local × (sale_rate − cost_rate)`, and the two reconstruct the total with no cross-term stranded — a test asserts the identity on four real production rows in both directions. A second defect surfaced while fixing the first: `realised_gain_local` was `realised_gain_dkk / sale_rate`, which is the DKK gain restated at the sale rate rather than the gain in local terms, making the split circular; it is now the genuine local figure. A startup backfill recomputes historical rows **from columns already stored on each row**, so it involves no rate lookup and cannot drift with today's FX. It derives the cost rate as `cost_basis_sold_dkk / cost_basis_sold_local` rather than reading `cost_basis_fx_rate_to_dkk`, deliberately: that column is 100x too small on pre-2026-07-09 rows, and trusting it would have invented ~+3,095 DKK of currency gain on one 2,356 DKK profit. It fails to the old behaviour rather than to nonsense — no usable local cost basis means the whole gain stays classified as price. A **plausibility guard** was added after the first backfill run exposed the real hazard: with a corrupt cost basis the split stays internally exact while being entirely fictional. Production holds derived cost rates of `128.2545` and `31.8992` against a ~7.02 sale rate, plus exact zeros; attributing currency from those would have produced large, confident, wrong numbers. A cost rate outside a 2x band either side of the sale rate is refused, because no currency in this book moves by half between purchase and sale. 41 of 45 sales attribute cleanly, 3 are refused as corrupt, 1 has no local basis. The FX cache-staleness half of this — `static_fx_rate_to_dkk` (`src/fx.rs:29`) hardcodes **USD at 7.0215** and was being reached silently on every conversion for over two days — was its own defect with its own root cause; **see U16, landed 2026-08-02.** **Measurement complete 2026-08-31.** `PositionExposure::currency_exposure_json` buckets invested value by currency and the Cash Deployment panel renders it beside the drawdown guardrail: share, position count, and totals, plus how many held symbols could not be valued or mapped. Shares are of *invested value* — that map holds positions plus active BUY reservations and no cash — and the scope string says so at both emission sites. The panel states the distinction the decision turns on: **`max_assets_per_currency` counts assets, not value.** Five USD positions can be five percent of the book or ninety, and only the second is the exposure. Unlike the guardrail rows beside it, the row is unconditional: those appear when they act, this one exists because currency was the exposure nothing showed at all, and a threshold-triggered row would need a threshold nobody has decided. **A second defect fell out of wiring it: `xome` had no currency.** U11 moved 20 symbols — the entire Swedish leg, a tenth of the universe — from `:xsto` to `:xome` and updated the Markov exchange-id map but not `currency_for_exchange`. Nothing failed loudly, which is why it survived a month: `order_currency` falls back to **DKK** for an unmapped exchange, and the currency concentration cap *refuses* rather than errors — so switching that cap on, the very lever this item asks about, would have blocked every BUY while any Swedish position was held. Both spellings now map to SEK, and a test reads the analysis universe out of both shipped configs and asserts every suffix it references has a currency. **Remaining: the policy choice only.** Whether currency becomes a gate input is the operator's call; the measurement it needs now exists, and note that a *count* cap would not have constrained the 63% USD book at all. |
...
| U12 | ~~The decision prompt has doubled in size and is mostly raw diagnostic data~~ **Root cause fixed 2026-08-03** | Average `prompt_text`: **271 KB (May) → 429 KB (June) → 527 KB (July)**, max 696 KB — roughly 130k+ tokens per call, twice a weekday. The bulk is Markov `recent_labels`: the latest prompt carries **1,240 individual daily observations** (close, regime, rolling_return) and 1,350 `"close"` values across 20 symbols. That array is UI diagnostic data; the decision inputs are the current state, the transition distribution, and the conviction, all of which are already present separately. This is a live cost line and a plausible decision-quality problem — the model is handed 62 days of per-day labels per symbol and asked for a judgement. | **Root cause identified 2026-08-03, not the mechanism first assumed.** `compact_markov_context` (`src/markov_method.rs`) already built a properly trimmed per-symbol `signals` list — no `recent_labels` in it. The bloat came from a second field, `latest_run`, which embedded the *entire* `markov_signal_runs.summary_json` row verbatim, and that row deliberately carries up to 20 full signal objects (including `raw_payload_json.recent_labels`) for operational debugging. Measured directly against the live production row: `summary_json` was **189,664 bytes**, of which **189,053 bytes (99.7%) was the embedded `signals` array** — 35% of the entire 527 KB average prompt from one nested field. `trim_markov_run_for_prompt` now strips only that array before the row reaches the prompt, keeping `status`/`run_id`/`asset_count`/`success_count`/`error_count`/`config` — the run-freshness metadata a model can actually use. Landed for all three consumers of `compact_markov_context` (the decision prompt, the MCP tool, and the Hermes evidence pack) since they share one function. Checked the two sibling modules for the same defect: `daily_indicators.rs`'s `compact_indicator_context` was **already clean** — its `latest_run` query never selects `summary_json` in the first place. `quiver.rs`'s `compact_quiver_context` has the same *pattern* (embeds the full `summary_json` including its own already-compact `signals`/`top_bullish`/`top_bearish` lists) but the real cost is small: measured at **10,418 bytes**, about 2% of the prompt — not worth a separate change yet, noted here rather than fixed. **Remaining:** confirm the ~35% reduction against a real decision-report `prompt_text` once the next weekday pulse runs on the new code (a live row is closer to authoritative than the isolated field measurement above), and separately measure win-rate/suggestion-quality before vs. after as originally planned — dropping the array should not change model input quality, since it was never in the per-symbol context the model actually reasons from, but that is a claim worth verifying rather than assuming. |
...
| U14 | ~~`audit_log` is 65 MB of dead Python exhaust — 38% of the database~~ **Landed 2026-08-03, operator-confirmed** | Largest table in the database. **Nothing had been written since 2026-05-10**, the Python→Rust cutover; every row was `broker_*_refreshed` spam from the legacy runtime (15,717 rows each for balance/positions/exposures/account, at ~1 row per 30s). `seq_scan = 0`, `idx_scan = 1` — the Rust runtime neither wrote nor read it. | **Dropped from the production primary (`daytrader-postgres-2`) 2026-08-03** after explicit operator confirmation — a destructive DB operation, not executed on the general "continue" instruction alone. Database size 172 MB → 109 MB. One prerequisite had to land first: `tune_append_heavy_table_autovacuum` (U13) ran `ALTER TABLE audit_log SET (...)` on every pod startup, so dropping the table without removing that entry first would have crash-looped every future pod restart or rollout — caught before dropping, fixed and deployed (`44a0945`) as its own step, verified clean, only then was the table dropped. Verified post-drop: table confirmed gone (`to_regclass` returns null), scheduler cycle still `ok`, no pod restarts, no errors in logs. No audit obligation existed to re-establish — nothing in the Rust runtime ever consumed this table's data. |
| U15 | Useful Saxo endpoints that would replace things we compute less reliably ourselves — **partially investigated 2026-08-03: the two most valuable ones are currently unusable, not because of access, but because of data** | Verified live against SIM: `/port/v1/closedpositions` and `/hist/v4/performance/timeseries` both return **200**. Neither is actually usable yet. `closedpositions` holds exactly **6 rows**, every one a `stop-test:*` closure from manual protective-stop testing on 2026-07-30/31 — nothing from the 39 other SELLs in `trade_ledger` going back to June, and `FromDate`/`ToDate` query parameters don't change the count, so this isn't a lookback-window setting, it's the account's actual recorded closed-position history. `hist/v4/performance/timeseries` is worse: its 5 data points run **2021-03-16 to 2021-03-22** — stale demo/provisioning data from Saxo's own SIM environment, entirely unrelated to this account's real 2026 trading period; zero points exist after 2026-06-01. Building a ledger cross-check or a guardrail-peak validator on either dataset right now would produce false-positive divergence alerts on data Saxo simply never recorded, not real findings — the exact failure mode this wiki has flagged elsewhere ("a panel that always warns is a panel nobody reads"). `/ca/v2/events` still returns 403 in SIM, so corporate actions remain unverified: **nothing in the runtime handles a split, dividend, or merger**, and a split would silently corrupt cost basis. Also unused: `/ref/v1/instruments/tradingschedule/{Uic}/{AssetType}` and the `mkt` service group. See [Unused Saxo Surface](#unused-saxo-surface). | Not actionable as originally scoped. `closedpositions` is worth revisiting once the SIM account has accumulated enough of its own real closed-position history post-reset to be a meaningful sample — the 6 stop-test rows prove the mechanism itself works, correctly, with real FIFO-matched P/L; there is just no backlog to check against yet. `performance/timeseries` looks unusable in SIM regardless of history depth and may need a LIVE-environment check instead, out of scope here. Corporate actions and trading schedule are unaffected by this finding and remain queued in that order. |
| U17 | ~~`get_context` handed Saxo `AccountKey` and `ClientKey` to Hermes~~ **Landed 2026-08-31** | Measured on the live MCP payload while investigating a truncation Hermes reported: **`AccountKey` appeared 25 times and `ClientKey` 20 times** in the document sent to an external model whose own prompt instructs it not to expose either. All 45 sat inside raw Saxo request/response documents nested at `execution.orders[].execution_result_json.broker_sync.broker_payload`, `.execution_result_json.payload`, `execution.events[].raw_payload_json.broker_payload` and `.raw_payload_json.payload`. No token, refresh token, or OAuth payload was present — the `safety` block's three existing claims were all true; the gap was that raw *broker* documents were never among the things it claimed to exclude. **Cause:** the Hermes context reuses the dashboard execution readers wholesale. The execution-order events API already allowlists its columns and excludes exactly these two, so the pattern existed and was simply never applied to this path. **Fix:** both columns are projected out for the Hermes path, dropped whole rather than redacted field by field, because the credentials sit at varying depths inside provider-shaped documents and a field-level denylist reopens the moment Saxo nests them somewhere new. `safety.raw_broker_payloads_excluded` now states the guarantee, and a test asserts no `AccountKey`/`ClientKey` survives the projection. Verified live: 0 occurrences of each. **The general lesson is the reuse, not the field.** A reader built for the dashboard carries whatever the dashboard is allowed to see; pointing an external-facing consumer at it inherits that allowance silently. See [Credential Hygiene](#credential-hygiene). |
...
**Second instance, 2026-08-31: broker credentials in the Hermes MCP context.**
See U17. Different in kind from the git-config token below — nothing was stored
wrongly, and no secret-bearing field was selected on purpose. A read model built
for the dashboard was reused for an external consumer, and it carried the raw
broker documents the dashboard is allowed to see. The check that would have
caught it is not "does this code touch credentials" but "who is allowed to see
what this reader returns, and is the new caller in that set". Both boundaries now
strip the raw documents, and the payload's own `safety` block asserts it.
...
**Decided and executed 2026-09-01.** The nine unreferenced scripts under `scripts/` were deleted after an exhaustive reference check: `check_saxo_order_status.py`, `create_clean_reconciliation_target.py`, `diagnose_saxo_instrument_lookup.py`, `import_sqlite_performance_history.py`, `reset_portfolio_baseline.py`, `stop_runtime.py`, `validate_strategy_journal.py`, `validate_swing_indicators.py`, `validate_swing_strategy.py`. **Six were already non-functional** rather than merely unused: five import `saxo_daytrader_xai`, deleted in `b43f0b7`, and `stop_runtime.py` manages `main.py`, `run_scheduler` and `web_main`, all deleted in `dfc77e0` — so they would have failed on import or found nothing to stop. The other three were superseded one-offs: a Positioner-import reconciliation builder, and two read-only Saxo diagnostics whose jobs the Rust runtime now does directly through execution-order events and the instrument negative cache. **Four scripts remain and all four are live:** `create_postgres_backup.py` and `prune_postgres_backups.py` are named individually in `Dockerfile.backup` and invoked by the two CronJobs, `saxo_oauth_helper.py` is documented in README, and `validate_execution_regressions.py` is referenced by the build-test-deploy runbook. `kubectl kustomize deploy/k8s/base` still builds and the backup image's two copied paths both still exist. The lesson worth keeping is that "unused" and "broken" were different questions here, and checking the second made the decision obvious: two thirds of the set could not have run at all.
