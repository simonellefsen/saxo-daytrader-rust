...
The audit implemented for U2 reports this automatically. After the 2026-07-26 automation-switch, exclusion, tax-estimate, risk-sizing, BUY-cost, candidate-ceiling, per-symbol exposure-cap, maximum-holdings, and post-gate BUY-selection-cap remediations, the shipped configuration has **34 enforced, 29 advisory, 31 unused, 14 of them risk-surface**, 0 uncontracted, 8 contracted-but-absent.

The remaining 14 keys that read as active risk controls and are not:

| Config key | Note |
| --- | --- |
| `strategy.max_assets_per_sector` | No concentration gate exists; the string `sector` does not appear anywhere in `src/`. |
| `strategy.min_selected_assets` | No minimum breadth floor is enforced. `strategy.max_selected_assets` became an enforced cap on distinct approved BUY symbols per Decision Report on 2026-07-26; SELLs and repeat BUYs for an already-selected symbol remain eligible. |
