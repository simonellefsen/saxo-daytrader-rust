...
| U2 | Config-contract audit | Several risk keys in `config.yaml` have zero references in `src/` — see [Unwired Risk Configuration](#unwired-risk-configuration). This is the same failure class as the retired 2026-05-05 `cash_buffer` override, but silent by construction: an unwired key is indistinguishable from an enforced one when reading the config. | Audit landed 2026-07-25 (`src/config_contract.rs`): every key is classified, unused risk keys appear in the Overview integrity payload, and a key added without a contract entry raises a warning. **Remediations landed 2026-07-26:** the strategy, Trading Manager, EU/US scheduled-pulse, `RISK_EXCLUDED_SYMBOLS`, Danish share-income estimate, ATR-based risk-per-trade, deterministic BUY cost controls, and the per-report candidate ceiling are now enforced, reducing the risk-surface inventory from 27 to 17 keys. **Remaining:** each of the 17 keys needs an operator decision to implement or delete. |
| U3 | Reconcile the Hermes goal contract with enforced reality | `AppState::hermes_goal_contract_value` sent Hermes `max_drawdown: 0.20`, `min_sharpe: 1.0`, `slippage_tolerance: 0.02`, and `max_positions` as *constraints*. Only the monthly-loss DKK floors were enforced in `src/trading_manager.rs`. Portfolio drawdown was computed but consumed only by display and evidence packs, so Hermes reasoned about a risk envelope the runtime did not apply. The return half of this was fixed 2026-07-25 — see [Return Goal](#return-goal). | Landed 2026-07-26. `src/drawdown_guard.rs` makes `max_drawdown` real: a soft band reduces the cycle BUY budget, a hard floor suspends new BUYs, SELLs are never blocked. The contract now reads its limit from `strategy.capital.drawdown_halt_pct`, the same key the gate applies, so the advertised and enforced numbers cannot drift. Every remaining objective and constraint carries an explicit `enforcement` status (`runtime_enforced` / `evaluation_only` / `structural` / `documentation` / `not_enforced`), and a test fails the build if a field is added without one. `gas_reserve` was deleted as a crypto-template leftover. **Remaining debt, now named rather than implied:** `max_positions`, `slippage_tolerance`, and `require_backtest_before_activation` are declared `not_enforced`. |
| U7 | Hermes can experiment on a dead variable | `deploy/k8s/base/hermes.yaml` listed `strategy.swing.cash_buffer_pct` in `experiment_policy.supported_variables`, and the config-contract audit proved nothing reads it. Hermes could therefore propose, run in SIM, observe, and promote a one-variable experiment whose variable had no effect — and attribute whatever the portfolio did to it. | Landed 2026-07-25, reconciled 2026-07-26: `strategy.swing.cash_buffer_pct` is removed from the Rust capabilities payload, Kubernetes ConfigMap, reflection prompts, runbooks, and the Trading Manager overlay loader. `SUPPORTED_EXPERIMENT_VARIABLES` is now the runtime source for overlay acceptance and is cross-checked against the config contract by test, so a variable classified `unused` cannot be offered or applied. Verified the guard rejects the retired path and other unpublished variables. |
...
The audit implemented for U2 reports this automatically. After the 2026-07-26 automation-switch, exclusion, tax-estimate, risk-sizing, BUY-cost, and candidate-ceiling remediations, the shipped configuration has **31 enforced, 30 advisory, 33 unused, 17 of them risk-surface**, 0 uncontracted, 8 contracted-but-absent.

The remaining 17 keys that read as active risk controls and are not:
