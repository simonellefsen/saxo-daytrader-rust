...
| U2 | Config-contract audit | Several risk keys in `config.yaml` have zero references in `src/` — see [Unwired Risk Configuration](#unwired-risk-configuration). This is the same failure class as the retired 2026-05-05 `cash_buffer` override, but silent by construction: an unwired key is indistinguishable from an enforced one when reading the config. | Audit landed 2026-07-25 (`src/config_contract.rs`): every key is classified, unused risk keys appear in the Overview integrity payload, and a key added without a contract entry raises a warning. **Remediations landed through 2026-07-27:** strategy automation, Trading Manager, EU/US scheduled pulses, `RISK_EXCLUDED_SYMBOLS`, Danish share-income estimate, ATR-based risk-per-trade, deterministic BUY cost controls, candidate and holding caps, the ladder per-symbol exposure cap, duplicate/inert cash-buffer, breadth, sector, and position-weight settings are resolved, reducing the risk-surface inventory from 27 to 7 keys. **Remaining:** each of the seven keys needs an operator decision to implement or delete. |
...
The audit implemented for U2 reports this automatically. After the 2026-07-26 automation-switch, exclusion, tax-estimate, risk-sizing, BUY-cost, candidate-ceiling, per-symbol exposure-cap, maximum-holdings, post-gate BUY-selection-cap, duplicate-cash-buffer retirement, explicit report-freshness-policy, Quiver-cadence, minimum-selection-floor, unsupported-sector-cap, and duplicate-position-weight retirements, the deployed configuration has **34 enforced, 30 advisory, 29 unused, 7 of them risk-surface**, 0 uncontracted. Local configuration carries the same Quiver policy rather than silently falling back to code defaults.

The remaining seven keys that read as active risk controls and are not:
