...
| U7 | Hermes can experiment on a dead variable | `deploy/k8s/base/hermes.yaml` lists `strategy.swing.cash_buffer_pct` in `experiment_policy.supported_variables`, and the config-contract audit proves nothing reads it. Hermes can therefore propose, run in SIM, observe, and promote a one-variable experiment whose variable has no effect — and attribute whatever the portfolio did to it. | Landed 2026-07-25: `strategy.swing.cash_buffer_pct` removed from the Rust capabilities payload and the Kubernetes ConfigMap. `SUPPORTED_EXPERIMENT_VARIABLES` is now a single constant cross-checked against the config contract by test, so a variable classified `unused` cannot be offered. Verified the guard fails when the dead path is re-added. |
...
**Consequence to watch.** `trail_stop_atr_multiple` (1.25) is tighter than `stop_loss_atr_multiple` (2.0), so a stop placed at the initial distance becomes ratchet-eligible immediately, before the position has appreciated at all. The twelve stops placed on 2026-07-25 at 2.0 ATR will therefore be rewritten to 1.25 ATR over the first few sweeps. That is what the configured pair means — give a new position room, tighten once it is protected — but it is a real tightening of risk posture that was not caused by any price move, and 1.25 ATR is close enough to daily noise on a swing horizon to be worth reviewing. Both multiples are single config values and the sweep's master switch is one boolean.
