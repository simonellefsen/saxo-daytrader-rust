...
def main() -> int:
    ...
    db_path = Path("/tmp") / f"saxo_daytrader_phase17_{uuid.uuid4().hex}.db"
    ...
    config["notifications"]["routes"]["weekly"] = {
        "slack_webhook_url": "https://hooks.slack.test/services/weekly",
    }
    ...
    assert calls.count("https://hooks.slack.test/services/default") == 1, calls
    assert calls.count("https://hooks.slack.test/services/weekly") == 1, calls
    assert calls.count("https://hooks.slack.test/services/alerts") == 1, calls
    assert len(sent_rows) == 3, sent_rows

    print("Phase 17 validation passed.")
    print(f"Imported source positions: {result.source_positions}")
    print(f"Excluded positions: {result.excluded_positions}")
    print(f"Routed deliveries sent: {len(sent_rows)}")
    print(f"Default webhook calls: {calls.count('https://hooks.slack.test/services/default')}")
    print(f"Override webhook calls: {calls.count('https://hooks.slack.test/services/weekly') + calls.count('https://hooks.slack.test/services/alerts')}")
