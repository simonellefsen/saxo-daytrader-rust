...
async fn create_daily_journal(
    ...
    let portfolio_baseline = portfolio_baseline_before_day(state, start_utc).await?;
    let portfolio = latest_portfolio_for_day(state, start_utc, end_utc).await?;
    let benchmark_readthrough = crate::performance_benchmarks::performance_benchmark_readthrough(
        state,
        portfolio_baseline.as_ref(),
        portfolio.as_ref(),
    )
    .await?;
    ...
    let diary_json = journal_diary_json(
        journal_date,
        &created_at,
        &summary,
        &metrics,
...
async fn portfolio_baseline_before_day(
    state: &AppState,
    start_utc: DateTime<Utc>,
) -> Result<Option<JsonValue>> {
    let row = sqlx::query(&format!(
        "SELECT recorded_at, total_market_value_dkk
         FROM portfolio_value_history
         WHERE recorded_at < '{}'
         ORDER BY recorded_at DESC, id DESC
         LIMIT 1",
    ...
    .fetch_optional(&state.pool)
    .await
    .context("loading portfolio baseline for strategy journal")?;
    Ok(row.map(|row| row_to_json(&row)))
}
...
fn journal_metrics(
    ...
    benchmark_readthrough: &JsonValue,
) -> JsonValue {
    ...
    json!({
        ...
        "benchmark_readthrough": benchmark_readthrough
    })
}
...
mod tests {
    ...
    #[test]
    fn daily_diary_embeds_only_the_read_only_benchmark_snapshot() {
        let benchmark_readthrough = json!({
            "status": "ready",
            "scope": "read_only_end_of_day_context_only",
            "portfolio_return_pct": 1.2,
        });
        let metrics = journal_metrics(&[], &[], None, &benchmark_readthrough);
        let diary = journal_diary_json(
        ...
        assert_eq!(
            diary
                .pointer("/diary/benchmark_readthrough/scope")
                .and_then(JsonValue::as_str),
            Some("read_only_end_of_day_context_only")
        );
        assert_eq!(
            metrics
                .pointer("/benchmark_readthrough/status")
                .and_then(JsonValue::as_str),
            Some("ready")
        );
    }
