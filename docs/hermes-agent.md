...
- Operator-confirmed provider/schema fallback retries use the failed report's exact persisted prompt with a supplied validated model and the current strict schema. Each retry is a separate `provider_fallback_dry_run` report, retains source-report provenance, and leaves the failure unchanged. Dry-run completions do not invoke Hermes or even read-only Saxo reference capture; they cannot reach Trading Manager, the queue, prechecks, or broker mutation.
...
`list_experiments` is an advisory-context projection, not the operator review queue: it returns only operator-approved or active lifecycle states. `pending_review` proposal values are omitted entirely and exposed only as a count, so an unreviewed threshold or setting cannot influence Hermes `allow`, `reduce`, `stand_down`, or `review` advice. Operators continue to review the full pending queue through the authenticated dashboard/API lifecycle surface.
...
2. **Completion-quality and Hermes advisory context:** the Rust completion boundary records a server-owned audit of report structure, scope, candidate evidence, capital context, canonical manager candidates, and execution metadata. Hermes receives only a compact audit status/count/check-label projection in its preflight metadata; it receives neither raw provider output nor raw diagnostics. A `ready` audit is not approval; a `review` item may support only a more conservative Hermes recommendation.
3. **Pulse authority and Trading Manager gates:** a report carries server-owned `pulse_mode` and `queue_eligible` fields. Only the explicit `execution_eligible` / `true` pair can reach the manager; `shadow` is denied before any queue write, even if a provider response later reaches `completed`. The manager then re-checks order shape, exchange status, cash buffer, circuit breakers, exclusions/quarantine, technical and Markov evidence, ATR risk, concentration, position caps, costs, and minimum trade value from local/broker-aware data.
4. **Execution queue gates:** only approved rows may reach the executor, which must be configured for Saxo execution. It claims a row once, validates session, market, whole-share quantity, sellable holdings, Saxo instrument/UIC, and permitted tick price before a request is built.
5. **Broker enforcement:** Saxo `/trade/v2/orders/precheck` is required before placement. A broker rejection or ambiguous network outcome is recorded for reconciliation; it is not blindly retried.

In `conservative` advisory mode Hermes can only `allow`, `reduce`, `stand_down`, or `review` a candidate that already exists. It cannot create a candidate, enlarge quantity, approve a broker order, access a Saxo session, invoke Saxo mutation endpoints, or use a pending-review experiment value as decision authority.
